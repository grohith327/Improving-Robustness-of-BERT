...
class USE(object):
    ...
    def semantic_sim(self, sents1, sents2):
        scores = self.sess.run(
            [self.sim_scores],
            feed_dict={self.sts_input1: sents1, self.sts_input2: sents2,},
        )
        return scores
...
class NLI_infer_BERT(nn.Module):
    ...
    def text_pred(self, text_data, batch_size=32):
        ...
        with torch.no_grad():
            for input_tokens, _ in dataloader:
        ...
        # print("probs_all", probs_all.size())
        # print('probs_all size:', probs_all.size())
        return probs_all


class NLIDataset_BERT(Dataset):
    def __init__(self, max_seq_length=64, batch_size=32):
    ...
    def transform_text(self, data, batch_size=32):
        ...
        eval_data = TensorDataset(input_tokens, input_tokens)
    ...
    if true_label != orig_label:
        ...
        for idx, synonyms in synonyms_all:
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                new_label_probs = (
                    new_probs[:, orig_label]
                    + torch.from_numpy(
                        (semantic_sims < sim_score_threshold)
                        + (1 - pos_mask).astype(float)
                    )
                    .float()
                    .cuda()
                )
